...
export function TemperatureChart({ readings }: Props) {
  ...
  const points = sorted.map((r) => `${toX(r.recordedAt).toFixed(1)},${toY(r.temperature).toFixed(1)}`).join(' ')
  ...
  return (
    <Card padding="none" className="overflow-hidden">
      <div className="px-4 pt-3 pb-1">
        <Text variant="small" weight="medium" style={{ color: 'var(--color-primary-400)' }}>
          Temperature (°C)
        </Text>
      </div>
              ...
              {sorted.length <= 60 &&
                sorted.map((r, i) => (
                  <circle
                    ...
                    cy={toY(r.temperature).toFixed(1)}
                    r="3"
